...
export default function PairAffinity() {
  const [page, setPage] = useState(1)
  const [searchNumbers, setSearchNumbers] = useState<string>('')

  const { data, isLoading } = trpc.analysis.pairAffinity.useQuery({
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h2 className="text-xl font-semibold">단짝</h2>
          <p className="text-sm text-slate-500 mt-1">최소 30회 이상 함께 나온 번호들</p>
        </div>
